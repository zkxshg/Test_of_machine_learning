...
def getWaterLine(equ, contour, aim, fileName):
    ...
    if not ifRealWater_0: 
        ...
        while conSize > 120 and minHpre <= Height_0 and minHpre >= minH:
            ...
            if isRealLine:
                ...
                # case 1：垂直下降,继续迭代
                if twid < (rightLvBound - leftLvBound) - 10 and min_ssd > dis_coef * zero_ssd:
                    minHpre = minHtem
                    # print("垂直下降")
                    continue
                elif min_ssd < dis_coef * zero_ssd and min_ssd - zero_ssd < 2:   # 标准差在误差范围内
                    if  Pixel_tem - Pixel_LB < dis_coef * zero_ssd:  # 像素差异在两个标准差内
                        minHpre = minHtem
                        # print("迭代结束: ", minHpre)
                        break
...
def run():
    while True: 
        try:
            ...
            leftCol, rightCol = cutPicByMouse(filePath + "/" + picNames[0])
            ...
            for pic in picNames:
                try:
                    ...
                    equ, contour = preProcess(aim)
                    getWaterLine(equ, contour, aim, fileName)
                except:
