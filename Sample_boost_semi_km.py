...
if __name__=='__main__':
    ...
    # labeled_data = 1.0 * (labeled_data - labeled_data.mean()) / labeled_data.std()  # 数据标准化
    '''
    ...
    while(abs(sse_diff) > 0.001) and iter_num < iteration:  # start iteration
        ...
        for p in range(0, attr_len):    # calculate the d(sse) of deleting attribute
            ...
            (p_label, labeled_2) = semi_km(data_p, labels, labeled_p, labeled, iteration, label_uni2)
            mean_p = update(data, p_label, k, labeled_data, labeled_2)
            model_p_sse = sse(data, p_label, labeled_data, labeled_2, mean_p)   # SSE after weighting
            sse_pa = last_sse-model_p_sse
            if sse_pa > max_dff:    # Select the model with min(sse)
                max_index = p
                max_dff = sse_pa
        if max_index != -1:
            w_a[max_index] = w_a[max_index] * n
        data_w = np.multiply(data, w_a)  # re-weighing and calculate the change of SSE
        labeled_w = np.multiply(labeled_data, w_a)
        (model_label, model_2) = semi_km(data_w, labels, labeled_w, labeled, iteration, label_uni2)
        mean_model = update(data, model_label, k, labeled_data, model_2)
        sse_w = sse(data, model_label, labeled_data, model_2, mean_model)
        acc_w = acc(model_label, labels, label_uni2, len(labeled_data))
        sse_diff = last_sse - sse_w
        last_sse = sse_w
    print(sse_w)
